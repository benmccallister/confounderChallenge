...
subtitle: "A Case Study in Causal Inference with Two Surgeons"
format:
  html: default
...
ax.scatter(dreamy_data['patient'], dreamy_data['post_surgical_score'], 
           marker='o', s=80, color=color_dreamy, label='Doc Dreamy', 
           alpha=0.7, edgecolors='black', linewidth=0.5)
ax.scatter(duck_data['patient'], duck_data['post_surgical_score'], 
           marker='^', s=100, color=color_duck, label='Doc Duck', 
           alpha=0.7, edgecolors='black', linewidth=0.5)
...
ax.text(5, mean_dreamy + 0.15, f'○ Doc Dreamy mean = {mean_dreamy:.2f}', 
        fontsize=11, color=color_dreamy, fontweight='bold', 
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8))
ax.text(5, mean_duck + 0.15, f'△ Doc Duck mean = {mean_duck:.2f}', 
        fontsize=11, color=color_duck, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8))

# Styling
ax.set_xlabel('Patient Number', fontsize=13, fontweight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', fontsize=13, fontweight='bold')
ax.set_title('Post-Surgical Symptom Score by Patient and Doctor', fontsize=15, fontweight='bold', pad=20)
...
# Add shaded region from -1 to 1 on x-axis (add first so it's behind everything)
ax.axvspan(-1, 1, alpha=0.15, color='gray', zorder=0, label='Overlap Region (-1 to 1)')
...
# Add horizontal mean lines
ax.axhline(y=mean_dreamy, color=color_dreamy, linestyle='--', linewidth=2, alpha=0.8, zorder=2)
ax.axhline(y=mean_duck, color=color_duck, linestyle='--', linewidth=2, alpha=0.8, zorder=2)

# Add text annotations with shape symbols
# Position them on the left side of the plot
ax.text(-2.5, mean_dreamy + 0.15, f'○ Doc Dreamy mean = {mean_dreamy:.2f}', 
        fontsize=11, color=color_dreamy, fontweight='bold', 
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8))
ax.text(-2.5, mean_duck - 0.25, f'△ Doc Duck mean = {mean_duck:.2f}', 
        fontsize=11, color=color_duck, fontweight='bold',
        bbox=dict(boxstyle='round,pad=0.5', facecolor='white', alpha=0.8))

# Styling
ax.set_xlabel('Initial Patient Severity', fontsize=13, fontweight='bold')
ax.set_ylabel('Post-Surgical Symptom Score (Lower is Better)', fontsize=13, fontweight='bold')
ax.set_title('Post-Surgical Symptom Score by Patient Severity', fontsize=15, fontweight='bold', pad=20)
ax.legend(loc='upper right', fontsize=11, frameon=True, shadow=True)
ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
...
The surgical outcomes paradox demonstrates how easily we can be misled by observational data when confounding variables are present. Patient severity operated as a hidden common cause, making Doc Dreamy appear superior when, in reality, Doc Duck was the better surgeon who simply treated more difficult cases.

Two powerful approaches allowed us to uncover the truth:

1. **Randomization** broke the confounding pathway by ensuring patient severity no longer influenced surgeon assignment, revealing Doc Duck's true superiority.

2. **Stratification** allowed us to compare surgeons within groups of similar patient severity, demonstrating that when we account for the confounder, Doc Duck consistently achieves better outcomes.

This case study reinforces a critical lesson in data analysis: correlation is not causation. When interpreting observational data, always consider what hidden variables might be influencing both treatment selection and outcomes. Without proper causal inference methods—whether through experimental design or statistical adjustment—we risk making precisely the wrong conclusion.
